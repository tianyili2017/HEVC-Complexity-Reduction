...
def get_accuracy_on_large_data_interqp(f, data_set, qp_thr_list, thr_list, plot_tag_set=None):
	...
	for i in range(len(qp_thr_list)+1):
		if count_list[i]>0:
			...
			accuracy,  _, _, _, accuracy_list, tendency_list = get_accuracy_on_large_data(f, data_set.images[stat_index[i]], data_set.qps[stat_index[i]], data_set.labels[stat_index[i]], thr_list=thr_list, is_get_y_predict=True, is_print=True, plot_tag='%s_%d'%(plot_tag_set,i))

def get_accuracy_on_large_data(f, images,qps,labels,thr_list,is_get_y_predict=False,is_print=False,plot_tag=None):
	...
	for i in range(math.ceil(length / PRE_BATCH_SIZE)):
		...
		if index_end > length:
			index_end = length
		y_predict_temp_64,y_predict_temp_32,y_predict_temp_16, accuracy_temp = sess.run([y_conv_64,y_conv_32,y_conv_16,accuracy_list],feed_dict={x:images[index_start:index_end], y_:input_data.translabel_matrix(labels[index_start:index_end]), qp:qps[index_start:index_end], is_train:0})
...
def evaluate_loss_accuracy(step,learning_rate_value):
	...
	train_y_predict_temp_64,train_y_predict_temp_32,train_y_predict_temp_16,train_accuracy_list_c, train_loss_list_c = sess.run([y_conv_64,y_conv_32,y_conv_16, accuracy_list, loss_list],feed_dict={x:train_batch[0], y_:train_batch[1], qp:train_batch[2], is_train:0})
	valid_y_predict_temp_64,valid_y_predict_temp_32,valid_y_predict_temp_16,valid_accuracy_list_c, valid_loss_list_c = sess.run([y_conv_64,y_conv_32,y_conv_16, accuracy_list, loss_list],feed_dict={x:valid_batch[0], y_:valid_batch[1], qp:valid_batch[2], is_train:0})
...
y_flat_64, y_flat_32, y_flat_16, y_conv_64, y_conv_32, y_conv_16, total_loss, loss_list, learning_rate_current, train_step, accuracy_list, opt_vars_all = input_data.nt.net(x, y_, qp, is_train, global_step, LEARNING_RATE_INIT, MOMENTUM_INIT, ITER_TIMES_PER_CHANGE_RATE, LEARNING_RATE_DECAY_RATIO, PARTLY_TUNING_MODE)
...
# the initial evaluate is only valid when start a new training (without fine tuning)
...
for i in range(ITER_TIMES):
	...
	if IS_RESET_LR == True:
		...
		feed_step = step
	learning_rate_value, _ = sess.run([learning_rate_current,train_step], feed_dict = {x:batch[0], qp:batch[2], y_:input_data.translabel_matrix(batch[1]), is_train:1, global_step:feed_step})
